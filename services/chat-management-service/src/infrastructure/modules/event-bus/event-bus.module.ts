...
export class EventBusModule implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    eventHandlerProviders.forEach(({ meta, discoveredClass }) => {
      this.eventBus.registerHandlers(
        (discoveredClass.instance as IEventHandlerProvider).getEventHandlers()
      );
    });
  }
  ...
  static createAsyncProviders(options: EventBusModuleAsyncOptions): Provider[] {
